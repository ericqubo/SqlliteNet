...
namespace SqlliteNetMallcoo
{
    ...
    /// </summary>
    public partial class SQLiteConnection : DbModelBuilder, IDisposable
    {
        private bool _open;
        ...
        /// </returns>
        public int CreateTablePre(Type T)
        {
            if (!HasExistEntity.Contains(T))
            {
                int r = CreateTable(T);
                if (r > -1)
                {
                    HasExistEntity.Add(T);
                }
                return r;
            }
            else
        ...
        void MigrateTable(TableMapping map)
        {
            ...
            foreach (var p in map.Columns)
            {
                ...
                {
                    found = (string.Compare(p.Name, c.Name, StringComparison.CurrentCultureIgnoreCase) == 0);
                    if (found)
                        break;
                }
                if (!found)
        ...
        private void CreateTables(List<Type> EntityList)
        {
            if (EntityList.Count > 0)
            {
                foreach (Type tp in EntityList)
                {
                    CreateTablePre(tp);
                }
                EntityList.Clear();
